# pip install flask
# 플라스크란?
# 파이썬으로 만든 db연동 콘솔 프로그램을 웹으로 연결하는 프레임워크이다.
...
#         플라스크 클래스,   프론트 연결,  요청에 응답을 줌, 주소 전달, 주소 생성, 상태 저장
...
@app.route('/login', methods=['GET','POST']) # http://localhost:5000/login # 이라는 경로를 만듦
    # methods는 웹에 동작에 관여함
    # GET은 url 주소로 데이터를 처리(보안상 좋지 않음)
    # POST: body 영역에서 데이터를 처리 (보안 상 좋음, 대용량에서 많이 사용함)
    # 대부분 처음에 화면(html 랜더)을 요청할 때는 GET 방식으로 처리
    ...
    try: # 예외발생 가능성 있음
        with conn.cursor() as cursor: # db에 커서 객체 사용
            ...
            if user:
                ...
                # 세션에 저장 완료
                # 브라우저에서 F12번을 누르고 애플리케이션 탭에서 쿠키 항목에 가면 session 객체가 보인다.
                # 이것을 삭제하면 로그아웃 처리 됨
...
@app.route('/logout') # 기본 동작이 get 방식이라, method=['GET'] 생략 가능
def logout():
...
def join(): # http://localhost:5000/ get 메서드(화면 출력) post(화면 폼 처리용)
    if request.method == 'GET':
        return render_template('join.html') # 로그인 화면용 프론트로 보냄

    # POST 매서드인 경우 (폼으로 데이터가 넘어올 때 처리)
    ...
    try: # 예외 발생 가능성이 있는 코드
        ...
        print(f"회원가입 에러: {e}")
        return "가입 중 에러가 발생했습니다. /n join()메서드를 확인하세요."
...
def member_edit():
    ...
    try:
        ...
        print(f"회원수정 에러: {e}")
        return f"회원 수정 중 오류가 발생했습니다. /n member_edit()메서드를 확인하세요."
...
@app.route('/mypage')
def mypage():
    if 'user_id' not in session: # 로그인 상태인지 확인
    ...
    try:
        with conn.cursor() as cursor:
            ...
            user_info = cursor.fetchone() # 찾아옹 값 1개를 user_info에 담음(dict)
            ...
            # 결과를 리턴한다.                         mypage.html에게 user객체와 board_count객체를 담아 보냄
...
@app.route('/') # url 생성용 코드 http://localhost:5000/ or http://192.168.0.179:5000
def index():
